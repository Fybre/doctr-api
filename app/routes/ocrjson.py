...
from app.schemas import OCRBlock, OCRIn, OCRLine, OCROut, OCRPage, OCRWord, OCRJsonOut
...
@router.post("/", response_model=OCRJsonOut, summary="Perform OCR")
async def perform_ocr(request: OCRIn = Depends(), files: list[UploadFile] = [File(...)]):
    ...
    result = OCRJsonOut(result=out.render())
    return result

@router.post("/single/", response_model=OCRJsonOut, summary="Perform OCR on single file - don't forget trailing /")
